...
void solve(int n) {
	...
	dy[0][0] = sticker[0][0];
	dy[1][0] = sticker[1][0];
	dy[0][1] = dy[1][0] + sticker[0][1];
	dy[1][1] = dy[0][0] + sticker[1][1];

	for (int i = 2; i < n; i++) {
		dy[0][i] = max(dy[1][i-2], max(dy[0][i - 2], dy[1][i - 1])) + sticker[0][i];

		dy[1][i] = max(dy[0][i-2], max(dy[0][i - 1], dy[1][i - 2])) + sticker[1][i];
	}


	cout << max(dy[0][n-1], dy[1][n-1]) << endl;
}
